...
pub fn begin(projection: *const mat4, view: *const mat4) void {
    ...
    gl.UniformMatrix4fv(shaders.debug_shader.u_projection, 1, gl.FALSE, @ptrCast(projection));
    gl.UniformMatrix4fv(shaders.debug_shader.u_view, 1, gl.FALSE, @ptrCast(view));
}

pub fn quad(model: *const mat4) void {
    gl.UniformMatrix4fv(shaders.debug_shader.u_model, 1, gl.FALSE, @ptrCast(model));
